...
const RichContentBlockSchema = new mongoose_1.default.Schema({
    title: {
        type: String,
        required: true,
        trim: true,
...
CourseDetailSchema.pre("validate", function autoNumberModules() {
    ...
    if (Array.isArray(this.aboutCourse)) {
        this.aboutCourse = this.aboutCourse.map((block, index) => ({
            ...block,
            order: typeof block.order === "number" && Number.isFinite(block.order)
                ? block.order
                : index,
        }));
    }
...
exports.CourseDetail = mongoose_1.default.model("CourseDetail", CourseDetailSchema);
